  ...
  var fetchSales = function fetchSales(ctx, callback) {
    _store__WEBPACK_IMPORTED_MODULE_1__["default"].dispatch('app-user/fetchUserSales', {
      ...
      response.data.sales.forEach(function (sale) {
        for (var key in sale) {
          console.log("GENT");
          console.log(sale.sale[0][key]);
          if (regex.test(sale.sale[key])) {
            sale.sale[key] = sale.sale[key].toFixed(2);
          }
        }
      });
